...
* stores the study tracker data i.e., all `Person` objects (which are contained in a `UniquePersonList` object).
...
| `* * *`  | user                                       | find a person by name          | locate details of study spots without having to go through the entire list |
| `* *`    | user                                       | hide private contact details   | minimize chance of someone else seeing them by accident                |
| `*`      | user with many study spots in the study tracker | sort study spots by name           | locate a person easily                                                 |
