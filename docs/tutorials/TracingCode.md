...
Recall from the User Guide that the `edit` command has the format: `edit INDEX [n/NAME] [r/RATING] [e/EMAIL] [a/ADDRESS] [t/TAG]…​` For this tutorial we will be issuing the command `edit 1 n/Alice Yeoh`.
    ...
    public JsonSerializableAddressBook(ReadOnlyAddressBook source) {
        persons.addAll(
            source.getPersonList()
                  .stream()
                  .map(JsonAdaptedStudySpot::new)
                  .collect(Collectors.toList()));
    }
    ```

1. It appears that a `JsonAdaptedStudySpot` is created for each `Person` and then added to the `JsonSerializableAddressBook`.
...
2.  What components will you have to modify to perform the following
    ...
    3.  Save the study tracker in the CSV format instead

    4.  Add a new command

    5.  Add a new field to `Person`

    6.  Add a new entity to the study tracker
